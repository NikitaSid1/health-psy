...
import DesktopMenu from "./DesktopMenu"; // <-- Подключили наше новое меню
...
export default function Header() {
  ...
  // Состояние для открытия/закрытия Бургер-меню на ПК
  ...
  return (
    <>
        ...
        <div id="header-container" className="layout-container flex items-center justify-between w-full h-16 md:h-20 px-4 md:px-8">
          ...
          <div id="header-left" className="flex items-center gap-4">
            <button 
              ...
              onClick={() => setIsDesktopMenuOpen(true)} // <-- Открываем меню
      ...
      {/* Рендерим компонент меню вне тега <header> для правильного наложения (z-index) */}
      <DesktopMenu 
