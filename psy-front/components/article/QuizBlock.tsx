// C:\Users\Admin\Desktop\psy\psy-front\components\article\QuizBlock.tsx
...
export default function QuizBlock({ title, questions }: QuizProps) {
  ...
  useEffect(() => {
    import(`@/dictionaries/${lang}.json`)
      .then((m) => setDict(m.default.quiz))
      .catch(() => import(`@/dictionaries/ru.json`).then((m) => setDict(m.default.quiz)));
  }, [lang]);
    ...
    return (
      <div className="my-12 p-8 rounded-[24px] bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/50 text-center animate-in fade-in zoom-in duration-500">
        ...
        <h4 className="text-xl font-bold text-green-900 dark:text-green-100 mb-2">
          {dict?.finishedTitle || "Test finished!"}
        </h4>
        <p className="text-green-700 dark:text-green-300">
          {dict?.finishedText || "Thank you for answering."}
        </p>
      </div>
    );
  }
    ...
    <div className="my-12 p-6 sm:p-8 rounded-[24px] bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 shadow-sm">
      ...
      <div className="mb-8">
        <div className="flex justify-between text-xs font-bold text-gray-400 mb-3">
          <span>{dict?.question || "Question"} {currentStep + 1}</span>
          <span>{dict?.of || "of"} {questions.length}</span>
        </div>
